...
import skylinkers.tn.mediconnectbackend.mapper.PatientMapper;
...
/**
 * SOLID — SRP: Handles only patient profile lifecycle.
 *              Auth is Keycloak's concern. Appointments belong to another service.
 * SOLID — OCP: New patient features extend PatientService interface without
 *              modifying this class (add to interface + implement here).
 */
@Service
...
public class PatientServiceImpl implements PatientService {
    ...
    public PatientResponse updateProfile(Long id, UpdateProfileRequest request) {
        ...
        String before = snapshotProfile(patient);
    ...
    private String snapshotProfile(Patient patient) {
        return "{"
                + "\"firstName\":\"" + safe(patient.getFirstName()) + "\","
                + "\"lastName\":\"" + safe(patient.getLastName()) + "\","
                + "\"phone\":\"" + safe(patient.getPhone()) + "\","
                + "\"address\":\"" + safe(patient.getAddress()) + "\","
                + "\"dateOfBirth\":\"" + String.valueOf(patient.getDateOfBirth()) + "\","
                + "\"gender\":\"" + String.valueOf(patient.getGender()) + "\","
                + "\"bloodType\":\"" + safe(patient.getBloodType()) + "\","
                + "\"allergies\":\"" + safe(patient.getAllergies()) + "\","
                + "\"emergencyContact\":\"" + safe(patient.getEmergencyContact()) + "\","
                + "\"profilePicture\":\"" + safe(patient.getProfilePicture()) + "\""
                + "}";
    }
